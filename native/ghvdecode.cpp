...
static void init_crc(){}static uint32_t crc32(const uint8_t* p,size_t n){return ghvcrc::compute(p,n);}static int8_t unzig(uint8_t z){int v=(int(z)>>1)^-int(z&1);return int8_t(v);}
...
int main(int argc,char** argv){
    if(argc<2){std::cerr<<"usage: ghvdecode INPUT.ghv [--start-frame N] [--verify] [--buffer-frames N] [--frames N] [--no-output] [--profile]\n";return 2;}
    std::string path=argv[1];uint64_t start_frame=0,limit=0;bool verify=false,no_output=false,profile_enabled=false;int buffer_frames=12;
    for(int i=2;i<argc;i++){std::string a=argv[i];if(a=="--verify")verify=true;else if(a=="--no-output")no_output=true;else if(a=="--profile")profile_enabled=true;else if(a=="--start-frame"&&i+1<argc)start_frame=std::stoull(argv[++i]);else if(a=="--buffer-frames"&&i+1<argc)buffer_frames=std::clamp(std::stoi(argv[++i]),1,64);else if(a=="--frames"&&i+1<argc)limit=std::stoull(argv[++i]);}
#ifdef _WIN32
    ...
    auto t0=std::chrono::steady_clock::now();uint64_t decoded=0,output=0,read_ns=0,output_ns=0,codec_ns=0,i_codec_ns=0,p_codec_ns=0,repeat_ns=0,verify_ns=0,i_count=0,p_count=0,repeat_count=0;ghvc8::Profile codec_profile;std::shared_ptr<std::vector<uint8_t>> prev;
    auto decode_next=[&](uint64_t i)->std::shared_ptr<std::vector<uint8_t>>{
        auto read_t0=std::chrono::steady_clock::now();uint8_t rh[32];f.read(reinterpret_cast<char*>(rh),32);if(f.gcount()!=32||std::memcmp(rh,"VFRM",4)!=0)throw std::runtime_error("bad frame header at "+std::to_string(i));uint32_t no=le32(rh+4);uint8_t typ=rh[16],codec=rh[17];uint16_t meta=le16(rh+18);uint32_t raw=le32(rh+20),packed=le32(rh+24),checksum=le32(rh+28);if(no!=i||(codec<4||codec>9)||raw!=frame_size)throw std::runtime_error("unsupported/corrupt frame "+std::to_string(i));std::vector<uint8_t> payload(packed);if(packed){f.read(reinterpret_cast<char*>(payload.data()),packed);if(size_t(f.gcount())!=packed)throw std::runtime_error("truncated payload");}read_ns+=uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now()-read_t0).count());std::vector<uint8_t> unwrapped;if(typ!=2&&codec<7)unwrap_zp06(payload,unwrapped);const std::vector<uint8_t>& work=(typ==2||codec>=7)?payload:unwrapped;int dx=int8_t(meta&255),dy=int8_t((meta>>8)&255);
        auto codec_t0=std::chrono::steady_clock::now();std::shared_ptr<std::vector<uint8_t>> recon;if(typ==2){if(!prev)throw std::runtime_error("repeat without prev");recon=prev;}else{std::vector<uint8_t> res;recon=std::make_shared<std::vector<uint8_t>>();if(codec==9){if(typ!=0&&typ!=1)throw std::runtime_error("bad GHVC9 frame type");ghvc9::decode(work,typ==1?prev.get():nullptr,int(w),int(ht),typ,frame_size,*recon,profile_enabled?&codec_profile:nullptr);}else if(codec==8){if(typ!=0&&typ!=1)throw std::runtime_error("bad GHVC8 frame type");ghvc8::decode(work,typ==1?prev.get():nullptr,int(w),int(ht),typ,frame_size,*recon,profile_enabled?&codec_profile:nullptr);}else if(codec==7){if(typ!=0&&typ!=1)throw std::runtime_error("bad GHVC7 frame type");ghvc7::decode(work,typ==1?prev.get():nullptr,int(w),int(ht),typ,frame_size,*recon);}else if(codec==6&&typ==1){if(!prev)throw std::runtime_error("P without prev");if(work.size()>=4&&std::memcmp(work.data(),"GPM6",4)==0){std::vector<int8_t> mvx,mvy;unpack_p6(work,int(w),int(ht),mvx,mvy,res,frame_size);block_motion_restore(res,*prev,*recon,int(w),int(ht),mvx,mvy);}else{unpack_gbp56(work,res,frame_size);motion_restore(res,*prev,*recon,int(w),int(ht),0,0);}}else{if(codec==6||codec==5)unpack_gbp56(work,res,frame_size);else unpack_gbp4(work,res,frame_size);if(typ==0)intra_restore(res,*recon,int(w),int(ht));else if(typ==1){if(!prev)throw std::runtime_error("P without prev");motion_restore(res,*prev,*recon,int(w),int(ht),dx,dy);}else throw std::runtime_error("bad frame type");}}uint64_t codec_delta=uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now()-codec_t0).count());codec_ns+=codec_delta;if(typ==0){i_codec_ns+=codec_delta;i_count++;}else if(typ==1){p_codec_ns+=codec_delta;p_count++;}else{repeat_ns+=codec_delta;repeat_count++;}if(verify){auto verify_t0=std::chrono::steady_clock::now();if(crc32(recon->data(),recon->size())!=checksum)throw std::runtime_error("CRC mismatch frame "+std::to_string(i));verify_ns+=uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now()-verify_t0).count());}prev=recon;decoded++;return recon;};
    try{
        if(no_output){for(uint64_t i=0;i<frame_count;i++){auto fr=decode_next(i);if(i>=start_frame){output++;if(limit&&output>=limit)break;}}}
            ...
            uint64_t written=0;while(true){FrameItem it;{std::unique_lock<std::mutex> lk(m);cv_not_empty.wait(lk,[&]{return done||!q.empty();});if(q.empty()&&done)break;it=std::move(q.front());q.pop_front();}cv_not_full.notify_one();auto output_t0=std::chrono::steady_clock::now();if(std::fwrite(it.frame->data(),1,it.frame->size(),stdout)!=it.frame->size()){producer.detach();return 8;}output_ns+=uint64_t(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now()-output_t0).count());written++;}producer.join();if(!err.empty())throw std::runtime_error(err);output=written;std::fflush(stdout);
        }
    }catch(const std::exception& e){std::cerr<<"decode error: "<<e.what()<<"\n";return 6;}
    double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();std::cerr<<"GHV_DECODE_RESULT decoded="<<decoded<<" output="<<output<<" elapsed="<<sec<<" fps="<<(decoded/std::max(sec,1e-6))<<" buffer="<<buffer_frames<<"\n";if(profile_enabled){auto ms=[](uint64_t ns){return double(ns)/1000000.0;};std::cerr<<"GHV_DECODE_PROFILE read_ms="<<ms(read_ns)<<" codec_ms="<<ms(codec_ns)<<" codec_i_ms="<<ms(i_codec_ns)<<" codec_p_ms="<<ms(p_codec_ns)<<" repeat_ms="<<ms(repeat_ns)<<" verify_crc_ms="<<ms(verify_ns)<<" motion_decode_ms="<<ms(codec_profile.motion_decode_ns)<<" coeff_decode_ms="<<ms(codec_profile.coeff_decode_ns)<<" inverse_recon_ms="<<ms(codec_profile.inverse_recon_ns)<<" output_ms="<<ms(output_ns)<<" i_frames="<<i_count<<" p_frames="<<p_count<<" repeats="<<repeat_count<<" zero_blocks="<<codec_profile.decode_zero_blocks<<" dc_blocks="<<codec_profile.decode_dc_blocks<<"\n";}return 0;
}
