...
import binascii, math, os, shutil, struct, subprocess, sys
from pathlib import Path
import numpy as np

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))
from ghv.container import read_header, read_index, FRAME_FMT, FRAME_SIZE, unpack_motion
from ghv.codec3 import decode_frame
from ghv.gha import parse_header as parse_gha, decode_pcm16le


def main():
    ghv_path = ROOT / 'tests' / 'sample_v04.ghv'
    ...
    with open(ghv_path, 'rb') as f:
        ...
        assert h.major == 0 and h.minor == 4 and h.frame_count == len(idx)
        for i, (off, typ_index) in enumerate(idx):
            ...
            assert no == i and typ == typ_index and codec == 3
    ...
    print(f'[PASS] GHV decode + CRC: {len(decoded)} frames')
    ...
    print(f'[PASS] GHA/GHAC1 decode: {frames} frames @ {sr} Hz / {ch} ch')
    ...
    print('[PASS] GHV 0.4 / GHA 0.2 self-test complete')
