...
setlocal
cd /d "%~dp0"
echo [GHV 0.7] Installing Python runtime dependencies...
...
if exist "native\bin\ghvcore.exe" if exist "native\bin\ghvdecode.exe" (
  echo [GHV] Native GHVC6/GHVC7 encoder and decoder already built.
  goto :done
)
where cl >nul 2>nul
...
goto :done
:build
echo [GHV] C++ compiler detected; building native GHVC6/GHVC7 encoder + decoder...
